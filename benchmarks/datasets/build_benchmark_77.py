...
FILLER_TEXTS = {
    "short": [
        {"left": "매우 아름답고 환상적인", "right": "초고화질의 사실적인 멋진 사진"},
        {"left": "최고 화질을 자랑하는 뛰어난", "right": "매우 정교하게 그려진 디지털 아트"},
        {"left": "가장 선명하고 밝은 톤의", "right": "정말 멋지고 깔끔한 고퀄리티 일러스트"}
    ],
    "near_limit": [
        {"left": "아름다운 풍경을 완벽하게 담아낸 놀라운 구성과 압도적인 디테일을 자랑하는 매혹적인 걸작으로 장엄한 분위기를 풍기는 매우 훌륭한 작품이며 빛의 부드러운", "right": "매우 사실적이고 디테일한 고화질 사진이며 최고 해상도의 텍스처와 조명 처리가 돋보이는 환상적인 마스터피스 수준의 놀라운 창작물로 모두에게 벅찬 감동을 주는"},
        {"left": "생동감 넘치는 색감을 보여주는 동시에 세밀한 선의 흐름과 깊이 있는 명암 대비를 통해 놀라운 통찰력이 담겨 있는 예술적인 묘사가 돋보이는 진정한", "right": "최고 수준으로 렌더링된 화려한 아트 작품으로 시각적 완성도가 극대화된 경이로운 창작물이며 모두의 시선을 사로잡는 완벽한 결과물로서 영원한 생명력을"},
        {"left": "시네마틱 조명과 질감을 묘사한 탁월한 감각이 빛을 발하는 환상적인 화면 구성으로 보는 이를 매료시키는 극적인 효과를 극대화한 독창적인 형태의 화려한", "right": "전문 작가의 손길이 느껴지는 일러스트레이션이며 다채로운 색채가 조화롭게 어우러져 깊은 영감을 선사하는 작품으로 세심한 디테일이 돋보이는 눈부신 예술적"}
    ],
    "over_limit": [
        {"left": "아주 아름답고 평화로운 풍경을 담아낸 매혹적인 걸작으로 전문 사진가가 찍은 듯한 완벽한 구도와 조명을 자랑하며 시선을 사로잡는 놀라운 몰입감을 제공하는 동시에 빛과 그림자의 완벽한 하모니를 통해 시간의 흐름조차 멈춘 듯한 영원의 순간을 캔버스 위에 고스란히 옮겨놓은 듯한 놀라운 감각을 뽐내는", "right": "매우 사실적이고 디테일이 살아있는 고화질 사진이며 최고 해상도의 질감과 부드러운 색채 그라데이션이 어우러져 깊이감을 극대화한 환상적인 시각적 경험을 제공하는 경이로운 예술 작품으로 피사체의 미세한 흔들림부터 배경의 광활한 공간감까지 모든 요소가 한 치의 오차도 없이 완벽한 역작으로서 역사에 길이 남을"},
        {"left": "환상적인 분위기와 생동감 넘치는 다채로운 색감을 보여주는 완벽한 구성의 역작으로 최신 3D 그래픽 엔진을 활용하여 정교하게 빚어낸 놀라운 입체감과 눈부신 광원 효과가 마치 마법처럼 어우러져 극강의 리얼리티를 구현해 냈으며 디테일 하나하나에 담긴 예술가의 열정이 폭발적으로 뿜어져 나오는 강렬하고 독보적인", "right": "최고 수준의 렌더링을 거친 화려한 디지털 아트이며 빛과 그림자의 대비가 돋보이는 시네마틱 효과와 세밀한 텍스처 묘사로 인해 눈앞에 생생하게 살아 움직이는 듯한 입체감을 선사하는 초현실적인 작품일 뿐만 아니라 철학적 메시지와 은유적 상징들이 겹겹이 쌓여 깊은 사유의 세계로 안내하는 영원불멸의 마스터피스"},
        {"left": "시네마틱 조명 아래 선명하고 부드러운 질감을 섬세하게 묘사한 압도적인 퀄리티의 명작으로 세세한 부분까지 놓치지 않고 완벽하게 그려낸 천재적인 붓터치와 타의 추종을 불허하는 색채 감각이 절묘한 조화를 이루어 캔버스 너머의 또 다른 차원으로 통하는 문을 열어젖힌 듯한 아찔한 미학적 쾌감을 선사하는 훌륭하고 멋진", "right": "전문 작가의 손길이 느껴지는 멋진 일러스트이며 역동적인 기법을 연상시키는 풍부한 색상 표현과 놀라운 명암비로 감상자의 마음을 강렬하게 울리는 깊은 여운과 감동을 자아내는 시각적 성취를 이루어냈고 무한한 상상력과 창조적 에너지가 결합되어 세상을 바라보는 새로운 창을 제시하는 놀랍고 대단한 미학적 성취의 결정체"}
    ]
}
...
def print_summary_report(records: List[Dict[str, str]]):
    total = len(records)
    print("\n" + "=" * 60)
    print("📊 Prompt Benchmark Dataset Generation Report (77 tokens limit) 📊")
    print("=" * 60)
...
def main():
    ...
    output_file = os.path.join(output_dir, "prompts_77.csv")

    print("Generating controlled prompt dataset (77 limit)...")
